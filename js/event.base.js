...
function event_base_save(obj){
	...
	var category_type_one = $("#category_type_one").val();
	var category_type_two = $("#category_type_two").val();
	...
	'"event_category1":"'+category_type_one+'",'+
	'"event_category2":"'+category_type_two+'",'+
	'"map":"'+map_point_info+'",'+
...
function load_event_base_page(event_id){
	...
	success:function(msg){
		if(msg.status == 0){
			...
			category_first = $.trim(categorys[0]);
			category_two = $.trim(categorys[1]);
			//	        	  var categorys = category.split(";");
			//	        	  $("#select_category_grade1").val(categorys[0]);
			//	        	  whenselectcategory();
			//	        	  $("#select_category_grade2").val(categorys[1]);
			$("#category_type_one").val(category_first);
			
			var getOptionIdvalue = $("#event_category_grade1 li[can='"+category_first+"']").html();
			$("#category_getOptionId_one").html(getOptionIdvalue);
            if(category_first!='0' && category_two!='0'){  
            	$("#category_select_required").val(category_first);
            	if(category_first=='12'){
            		$("#fenlei_two").hide();
            	}else{
            		event_category_zong(2,category_first,2,category_two);
            	}            	
            }else if(category_first!='0'){  
            	$("#category_select_required").val(category_first);
            	event_category_zong(2,category_first);
            }
			//咨询电话
 ...
 * author:zyc
 * time:2014年10月9日
 * 功能：活动分类变为二级  te  用于 特殊情况下的处理
 * */
function event_category_zong(level,parent_id,te,te_id){
	 var url=js_huodongshu_domain+"/event/getEventCategoryXiu.do";
	     if(level==''){
	    	 alert('活动级别不能为空');
	     }
	    var n={};
...
//	      			//新分类数据
		        	  if(level==1){	  
		      			...
		      			$("#event_category_grade2").html(html);
		      			UIselect();
		      			if(te==2){   //特殊情况处理用于一开始就选中的数据（二级菜单）
		      			   	$("#category_type_two").val(category_two);
		        			var getOptionIdvalue = $("#event_category_grade2 li[can='"+category_two+"']").html();
		        			$("#category_getOptionId_two").html(getOptionIdvalue);    	
		      			}
		        	  }
	              }else{
...
function myformvalidate(){
	...
	jQuery.validator.addMethod('category_select_required',
			function(){
				var category_select_type =  $("#category_select_required").val(); //alert(category_select_type);
	...
	$("#evnet_base_validateform").validate({
		...
		messages: {
			...
			'event_name': {
				...
				maxlength:"<span class='icon'></span><font color='red'>活动名称不能超过60个汉字！<font>"
			},
			//			'event_limit_num': {
...
//一级分类
function categoryMethodOneSelect(e){
	$("#category_getOptionId_one").attr("category_select_type",e.attr("can"));
	var val = e.attr("can");
	$("#category_type_one").val(val);
	$("#category_select_required").val(val);
	$("#category_type_two").val('0');
	$("#category_getOptionId_two").html('--请选择--');
	//活动类型  sl
	if(val > 0){
		//$("#ul_category_type_error").empty();
		$("#event_category_grade2").html('');
		if(val==12){
			$("#fenlei_two").hide();
		}else{
			$("#fenlei_two").show();
			event_category_zong(2,val);
		}
...
//二级分类
function categoryMethodTwoSelect(e){
	$("#category_getOptionId_two").attr("category_select_type",e.attr("can"));
	var val = e.attr("can");
	$("#category_type_two").val(val);
	//$("#category_select_required").val(val);
	//活动类型  sl
	if(val > 0){
		$("#ul_category_type_error").empty();
